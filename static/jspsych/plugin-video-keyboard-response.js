var jsPsychVideoKeyboardResponse = (function (jspsych) {
  'use strict';
// NOTE: This function has been modified by Vishal on Oct. 12 2023 to obtain series of responses rather than just one single response.
  const info = {
  ...
  class VideoKeyboardResponsePlugin {
      ...
      `);
          ...
          // store response
          var response = {
              rt: [], // <- changed from null to empty arrays, so that you can store multiple values (by Vishal)
              key: [] // <- changed from null to empty arrays, so that you can store multiple values (by Vishal)
          };
          ...
          const end_trial = () => {
              ...
              var trial_data = {
                  rt: JSON.stringify(response.rt), // previously only response.rt, (by Vishal)
                  stimulus: trial.stimulus,
                  response: JSON.stringify(response.key) // previously only response.key, (by Vishal)
              };
              // clear the display
          ...
          var after_response = (info) => {
              ...
              display_element.querySelector("#jspsych-video-keyboard-response-stimulus").className +=
                  " responded";
            // only record the first response (removed by Vishal)
            //   if (response.key == null) {
            //       response = info;
            //   }

              // record a series of responses
			  response.rt.push(info.rt);   // <- these two lines instead of "if (response.key == null) {response = info;}" (by Vishal)
          ...
          if (trial.choices != "NO_KEYS" && trial.response_allowed_while_playing) {
              this.jsPsych.pluginAPI.getKeyboardResponse({
                  ...
                  valid_responses: trial.choices,
                  rt_method: "performance",
                  persist: true, // <- changed from false to true by Vishal
                  allow_held_key: false,
              });
          }
